...
from app.database.models import Note, Reminder, Task, User
...
Ты работаешь строго в рамках данных авторизованного пользователя.
...
class OpenClawAgent:
        ...
        return [
            {
                ...
                "function": {
                    "name": "create_task",
                    "description": "Создать задачу в БД пользователя",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "title": {"type": "string"},
            ...
            },
            {
                ...
                "function": {
                    "name": "create_note",
                    "description": "Создать заметку в БД пользователя",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "title": {"type": "string"},
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": "Создать напоминание в БД пользователя",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "title": {"type": "string"},
            ...
            {"type": "function", "function": {"name": "list_tasks", "description": "Получить список задач пользователя", "parameters": {"type": "object", "properties": {}}}},
            {"type": "function", "function": {"name": "list_notes", "description": "Получить список заметок пользователя", "parameters": {"type": "object", "properties": {}}}},
            {"type": "function", "function": {"name": "list_reminders", "description": "Получить список напоминаний пользователя", "parameters": {"type": "object", "properties": {}}}},
            {"type": "function", "function": {"name": "get_dashboard", "description": "Получить личную сводку пользователя", "parameters": {"type": "object", "properties": {}}}},
        ]

    def _execute_tool(self, db: Session, user: User, name: str, arguments: dict[str, Any]) -> dict[str, Any]:
        if name == "create_task":
            item = create_task(
                db,
                user,
        ...
        if name == "create_reminder":
            remind_at = parse_datetime(arguments.get("remind_at")) or (datetime.now(UTC) + timedelta(hours=1))
        ...
        if name == "list_tasks":
            return {"tasks": [serialize_model(item) for item in list_tasks(db, user)]}
        if name == "list_notes":
            return {"notes": [serialize_model(item) for item in list_notes(db, user)]}
        if name == "list_reminders":
            return {"reminders": [serialize_model(item) for item in list_reminders(db, user)]}
    ...
    async def run(self, db: Session, user: User, user_text: str) -> dict[str, Any]:
        if self.is_configured():
            return await self._run_llm_mode(db, user, user_text)
        return self._run_fallback_mode(db, user, user_text)

    async def _run_llm_mode(self, db: Session, user: User, user_text: str) -> dict[str, Any]:
        messages: list[dict[str, Any]] = [
            {"role": "system", "content": SYSTEM_PROMPT},
            {"role": "system", "content": f"Текущий пользователь: telegram_id={user.telegram_id}, username={user.username or ''}"},
        ...
        for _ in range(4):
            ...
            for tool_call in tool_calls:
                ...
                result = self._execute_tool(db, user, function["name"], arguments)
    ...
    def _run_fallback_mode(self, db: Session, user: User, user_text: str) -> dict[str, Any]:
        ...
        if parsed["type"] == "task":
            ...
            remind_at = parsed["datetime"] or (datetime.now(UTC) + timedelta(hours=1))
...
def parse_text(text: str) -> dict[str, Any]:
    ...
    if "завтра" in lowered:
        result["datetime"] = datetime.now(UTC) + timedelta(days=1)
    elif "сегодня" in lowered:
        result["datetime"] = datetime.now(UTC) + timedelta(hours=2)
...
def get_dashboard(db: Session, user: User) -> dict[str, Any]:
    user_tasks = list_tasks(db, user)
    user_notes = list_notes(db, user)
    user_reminders = list_reminders(db, user)
    return {
        "user": {
            "id": user.id,
        ...
        "recent_tasks": [serialize_model(item) for item in user_tasks[:5]],
        "recent_notes": [serialize_model(item) for item in user_notes[:5]],
        "upcoming_reminders": [serialize_model(item) for item in user_reminders[:5]],
    }
